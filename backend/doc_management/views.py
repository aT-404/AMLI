...
class DocumentRevisionViewSet(BaseModelViewSet):
    ...
    def _default_fallback_logo(self):
        try:
            import base64, os
            from django.conf import settings
            logo_path = os.path.join(settings.BASE_DIR, "..", "logo", "Max_Life_Insurance_logo.svg")
            if os.path.exists(logo_path):
                with open(logo_path, "rb") as f:
                    return base64.b64encode(f.read()).decode("utf-8"), "image/svg+xml"
        except Exception:
            pass
        return "", ""

    def _client_logo(self):
        """Return (base64, mime_type) of the org logo, falling back to default Max Life Insurance logo."""
        try:
            ...
            pass
        return self._default_fallback_logo()
