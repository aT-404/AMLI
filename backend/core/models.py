...
class LibraryMixin(ReferentialObjectMixin, I18nObjectMixin):
    ...
    copyright = models.CharField(
        max_length=4000, null=True, blank=True, verbose_name=_("Copyright")
    )
    ...
    def store_library_content(
        ...
        if not match_urn(urn):
            ...
            if len(parts) == 4 and parts[0] == "urn":
                urn = f"urn:{parts[1]}:compliance:{parts[2]}:{parts[3]}"
                library_data["urn"] = urn
        if not match_urn(urn):
...
class LibraryDraft(NameDescriptionMixin, FolderMixin):
    ...
    copyright = models.CharField(
        max_length=4000, blank=True, null=True, verbose_name=_("Copyright")
    )
...
class LoadedLibrary(LibraryMixin):
    ...
    def reference_count(self) -> int:
        """
        Returns the number of distinct dependent libraries, active ongoing risk/compliance assessments,
        control assignments, and requirement assessments that reference objects from this library.
        An audit is active if its audit_end_date and due_date have not passed (are in the future or not set).
        When an audit is deleted or its end date / due date has passed, the framework lock is lifted.
        """
        ...
        is_ended = Q(due_date__isnull=False, due_date__lte=today) | Q(eta__isnull=False, eta__lte=today)
        ongoing_ca = (Q(due_date__isnull=True) | Q(due_date__gt=today)) & (Q(eta__isnull=True) | Q(eta__gt=today))
        ...
        return (
            ...
            + ComplianceAssessment.objects.filter(
                ...
                | Q(requirement_assessments__requirement__framework__library=self)
                | Q(
            ...
            + ControlAssignment.objects.filter(
                Q(
                    requirement_node__framework__library=self,
                    is_active=True,
                )
                & (
                    Q(spoc_user_id__isnull=False)
                    | Q(reviewer_user_id__isnull=False)
                )
            )
            .filter(
                Q(framework__complianceassessment__in=ComplianceAssessment.objects.filter(ongoing_ca))
                | Q(requirement_node__framework__complianceassessment__in=ComplianceAssessment.objects.filter(ongoing_ca))
            )
            .exclude(due_date__isnull=False, due_date__lte=today)
            .distinct()
            .count()
...
class HistoricalMetric(models.Model):
    ...
    model = models.CharField(max_length=255, verbose_name=_("Model"), db_index=True)
...
class ComplianceAssessment(Assessment):
    ...
    class ScopeMode(models.TextChoices):
        FULL = "full", _("Full Framework")
        CUSTOM = "custom", _("Custom Scope")

    class AuditPhase(models.TextChoices):
        DRAFT = "DRAFT", _("Draft")
        EVIDENCE_COLLECTION = "EVIDENCE_COLLECTION", _("Evidence Collection")
        UNDER_REVIEW = "UNDER_REVIEW", _("Under Review")
        AUDIT_IN_PROGRESS = "AUDIT_IN_PROGRESS", _("Audit In Progress")
    ...
    evidence_due_date = models.DateField(
        null=True, blank=True, verbose_name=_("Evidence due date")
    )
    audit_end_date = models.DateField(
        null=True, blank=True, verbose_name=_("Audit end date")
    )
    audit_phase = models.CharField(
        max_length=30,
        choices=AuditPhase.choices,
        default=AuditPhase.DRAFT,
        verbose_name=_("Audit phase"),
        db_index=True,
    )
    active_snapshot_version = models.IntegerField(
        default=1, verbose_name=_("Active snapshot version")
    )
    completed_snapshot_version = models.IntegerField(
        null=True, blank=True, verbose_name=_("Completed snapshot version")
    )
    audit_period_year = models.CharField(
        max_length=100, null=True, blank=True, verbose_name=_("Audit period")
    )
    selected_nodes = models.JSONField(
        blank=True, null=True, verbose_name=_("Selected requirement nodes")
    )
    selected_domains = models.JSONField(
        blank=True, null=True, verbose_name=_("Selected domains")
    )
    schedule_type = models.CharField(
        max_length=20,
        choices=ScheduleType.choices,
        default=ScheduleType.ON_DEMAND,
    ...
    def create_requirement_assessments(
        ...
        if self.scope_mode == self.ScopeMode.FULL and self.framework is not None:
        ...
        elif self.scope_mode == self.ScopeMode.CUSTOM and self.selected_nodes and isinstance(self.selected_nodes, list):
            from django.db.models import Q
            unique_nodes = list(dict.fromkeys(self.selected_nodes))
            requirements = RequirementNode.objects.filter(
                Q(pk__in=unique_nodes) | Q(urn__in=unique_nodes)
            ).select_related()
            questions = Question.objects.filter(
                requirement_node__in=requirements
            ).select_related("requirement_node")
        else:
            requirements = RequirementNode.objects.none()
            questions = Question.objects.none()

        # If there's a baseline, prefetch all related baseline assessments and answers in one query
        baseline_assessments = {}
        baseline_answers = {}
        if baseline and ((baseline.scope_mode == self.ScopeMode.FULL and baseline.framework == self.framework) or baseline.scope_mode == self.ScopeMode.CUSTOM):
            baseline_assessments = {
        ...
        # Auto-match unexpired evidences from Evidence Library
        try:
            from django.utils import timezone
            from django.db.models import Q
            unexpired_evidences = list(
                Evidence.objects.filter(
                    Q(expiry_date__isnull=True) | Q(expiry_date__gte=timezone.now().date())
                )
            )
            if unexpired_evidences:
                evidence_throughs = []
                for assessment in created_assessments:
                    req = assessment.requirement
                    req_ref = (req.ref_id or "").lower().strip()
                    req_name = (req.name or "").lower().strip()
                    for ev in unexpired_evidences:
                        ev_name = (ev.name or "").lower()
                        ev_desc = (ev.description or "").lower()
                        if (req_ref and req_ref in ev_name) or (req_ref and req_ref in ev_desc) or (req_name and req_name in ev_name):
                            evidence_throughs.append(
                                RequirementAssessment.evidences.through(
                                    requirementassessment_id=assessment.id,
                                    evidence_id=ev.id
                                )
                            )
                if evidence_throughs:
                    RequirementAssessment.evidences.through.objects.bulk_create(
                        evidence_throughs, batch_size=1000, ignore_conflicts=True
                    )
        except Exception:
            pass
    ...
    def sync_requirement_assessments(self):
        """
        Synchronizes RequirementAssessment and AssessmentControlSnapshot records
        when audit controls or scope selection are modified.
        """
        from django.db import transaction
        from django.db.models import Q
        with transaction.atomic():
            if self.scope_mode == self.ScopeMode.FULL and self.framework is not None:
                desired_requirements = list(RequirementNode.objects.filter(
                    framework=self.framework
                ))
            elif self.scope_mode == self.ScopeMode.CUSTOM and self.selected_nodes and isinstance(self.selected_nodes, list):
                unique_nodes = list(dict.fromkeys(self.selected_nodes))
            ...
            desired_req_ids = {r.id for r in desired_requirements}

            # Delete RequirementAssessment objects for requirements no longer in scope
            RequirementAssessment.objects.filter(
                compliance_assessment=self
            ).exclude(requirement_id__in=desired_req_ids).delete()

            # Delete AssessmentControlSnapshot objects no longer in scope
            from core.models import AssessmentControlSnapshot
            AssessmentControlSnapshot.objects.filter(
                compliance_assessment=self
            ).exclude(requirement_node_id__in=desired_req_ids).delete()

            # Find existing requirement IDs
            ...
            # Requirements to add
            new_requirements = [r for r in desired_requirements if r.id not in existing_req_ids]
            ...
            if new_requirements:
                ...
                created_ras = RequirementAssessment.objects.bulk_create(ras_to_create)

                # Create empty Answers for new requirement questions
                questions = Question.objects.filter(requirement_node__in=new_requirements)
                ra_by_req = {ra.requirement_id: ra for ra in created_ras}
                answers_to_create = []
                for q in questions:
                    ra = ra_by_req.get(q.requirement_node_id)
                    if ra:
            ...
            # Refresh AssessmentControlSnapshot entries
            from core.audit_lifecycle import create_snapshots, auto_populate_evidence
            create_snapshots(self, version=self.active_snapshot_version or 1)
            auto_populate_evidence(self)

            self.update_auto_status()

    def update_auto_status(self):
        """Automatically update audit status in MySQL based on requirement response progress."""
        from django.db.models import Q
        total = RequirementAssessment.objects.filter(compliance_assessment_id=self.id).count()
        if total == 0:
            return
        answered = RequirementAssessment.objects.filter(compliance_assessment_id=self.id).filter(
            ~Q(result="not_assessed") | Q(evidences__isnull=False) | ~Q(status="to_do")
        ).distinct().count()
        ...
        if self.status != new_status:
            ...
            super(Assessment, self).save(update_fields=["status", "updated_at"])
    ...
    def get_selected_implementation_groups(self):
        ...
        if (
            not framework
            or not framework.implementation_groups_definition
...
class RequirementAssessment(AbstractBaseModel, FolderMixin, ETADueDateMixin):
    ...
    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        if self.compliance_assessment_id:
            try:
                self.compliance_assessment.update_auto_status()
            except Exception:
                pass
...
auditlog.register(FeatureToggle, exclude_fields=common_exclude)
...
class IntermediaryFolder(AbstractBaseModel):
    class FolderType(models.TextChoices):
        ROOT = "ROOT", "Root Repository"
        YEAR = "YEAR", "Year"
    ...
    folder_type = models.CharField(
        max_length=20, choices=FolderType.choices, default=FolderType.YEAR
    )
    parent = models.ForeignKey(
        "self",
        on_delete=models.CASCADE,
    ...
    @classmethod
    def get_root_repository(cls):
        root, _ = cls.objects.get_or_create(
            folder_type=cls.FolderType.ROOT,
            parent=None,
            defaults={"name": "Report Repository", "description": "Root Report Repository"},
        )
        return root


class IntermediaryDomainAssignment(AbstractBaseModel):
    domain_folder = models.ForeignKey(
        IntermediaryFolder,
        on_delete=models.CASCADE,
        related_name="assignments",
    )
    spoc_users = models.ManyToManyField(
        User, related_name="spoc_domain_assignments", blank=True
    )
    approving_admins = models.ManyToManyField(
        User, related_name="approving_admin_domain_assignments", blank=True
    )

    def __str__(self):
        spocs = ", ".join([u.email for u in self.spoc_users.all()]) if self.pk and self.spoc_users.exists() else "None"
        reviewers = ", ".join([u.email for u in self.approving_admins.all()]) if self.pk and self.approving_admins.exists() else "None"
        return f"Domain '{self.domain_folder.name}' (Reviewers: {reviewers} | SPOCs: {spocs})"


class IntermediaryPartnerReport(AbstractBaseModel):
    ...
    partner_folder = models.ForeignKey(
        IntermediaryFolder,
        on_delete=models.CASCADE,
    ...
    submitted_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="submitted_partner_reports"
    )
    compliance_status = models.CharField(
        max_length=20, choices=ComplianceStatus.choices, default=ComplianceStatus.COMPLIANT
    )
    approval_status = models.CharField(
        max_length=20, choices=ApprovalStatus.choices, default=ApprovalStatus.PENDING_APPROVAL
    )
    reviewer_feedback = models.TextField(blank=True, default="")
    approved_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="approved_partner_reports"
    )
    ...
    def __str__(self):
        return f"{self.title} - {self.partner_folder.name} ({self.compliance_status})"


class IntermediaryGeneratedReport(AbstractBaseModel):
    title = models.CharField(max_length=255)
    scope_type = models.CharField(max_length=20)
    scope_name = models.CharField(max_length=255)
    scope_id = models.CharField(max_length=100, blank=True, default="")
    reporting_period = models.CharField(max_length=255, blank=True, default="")
    metrics_snapshot = models.JSONField(default=dict, blank=True)
    generated_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="generated_compliance_reports"
    )
    file = models.FileField(upload_to="intermediary_reports/", null=True, blank=True)

    class Meta:
        ordering = ["-created_at"]

    def __str__(self):
        return f"{self.title} ({self.scope_name})"
...
auditlog.register(IntermediaryFolder, exclude_fields=common_exclude)
auditlog.register(IntermediaryDomainAssignment, m2m_fields={"spoc_users", "approving_admins"}, exclude_fields=common_exclude)
auditlog.register(IntermediaryPartnerReport, exclude_fields=common_exclude)
from iam.models import FeatureToggle
auditlog.register(FeatureToggle, exclude_fields=common_exclude)


# ==============================================================================
# CONTROL ASSIGNMENT, EVIDENCE MANAGEMENT, NOTIFICATION & ESCALATION MODELS
# ==============================================================================
...
class AllManager(models.Manager):
    """Manager that includes soft-deleted records for historical queries."""
    pass


class ControlAssignment(AbstractBaseModel, SoftDeleteMixin):
    requirement_node = models.ForeignKey(
        RequirementNode, on_delete=models.CASCADE, related_name="control_assignments"
    )
    framework = models.ForeignKey(
        Framework, on_delete=models.CASCADE, related_name="control_assignments"
    )
    spoc_user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="spoc_control_assignments"
    )
    reviewer_user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="reviewer_control_assignments"
    )
    assigned_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_control_assignments"
    )
    notes = models.TextField(blank=True, default="")
    pending_email_notification = models.BooleanField(default=True)
    due_date = models.DateField(null=True, blank=True)
    l1_reminder_date = models.DateField(null=True, blank=True)
    l2_grace_deadline = models.DateField(null=True, blank=True)
    l3_final_deadline = models.DateField(null=True, blank=True)
    escalation_level = models.CharField(max_length=30, default="ASSIGNED")
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["framework", "spoc_user"]),
            models.Index(fields=["framework", "reviewer_user"]),
            models.Index(fields=["spoc_user", "is_active"]),
            models.Index(fields=["reviewer_user", "is_active"]),
            models.Index(fields=["spoc_user", "pending_email_notification"]),
        ]


class AssignmentBatch(AbstractBaseModel):
    class Status(models.TextChoices):
        OPEN = "OPEN", _("Open")
        DISPATCHED = "DISPATCHED", _("Dispatched")

    event_id = models.CharField(max_length=255, unique=True)
    spoc_user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="assignment_batches")
    ...
    class Meta:
        indexes = [
            models.Index(fields=["spoc_user", "status"]),
            models.Index(fields=["status", "window_expires_at"]),
        ]


class AssignmentBatchItem(AbstractBaseModel):
    batch = models.ForeignKey(AssignmentBatch, on_delete=models.CASCADE, related_name="items")
    control_assignment = models.ForeignKey(ControlAssignment, on_delete=models.CASCADE, related_name="batch_items")

    class Meta:
        unique_together = [("batch", "control_assignment")]


class EvidenceRequirement(AbstractBaseModel, SoftDeleteMixin):
    control_assignment = models.ForeignKey(
        ControlAssignment, on_delete=models.CASCADE, related_name="evidence_requirements"
    )
    name = models.CharField(max_length=255)
    description = models.TextField(blank=True, default="")
...
class ControlEvidenceMapping(AbstractBaseModel, SoftDeleteMixin):
    control_assignment = models.ForeignKey(
        ControlAssignment, on_delete=models.CASCADE, related_name="evidence_mappings"
    )
    evidence_requirement = models.ForeignKey(
        EvidenceRequirement, on_delete=models.SET_NULL, null=True, blank=True, related_name="evidence_mappings"
    )
    evidence = models.ForeignKey(
        Evidence, on_delete=models.CASCADE, related_name="control_mappings"
    )
    mapped_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="mapped_evidences"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["control_assignment", "is_active"]),
        ]


class AssessmentControlSnapshot(AbstractBaseModel, SoftDeleteMixin):
    compliance_assessment = models.ForeignKey(
        ComplianceAssessment, on_delete=models.CASCADE, related_name="control_snapshots"
    )
    control_assignment = models.ForeignKey(
        ControlAssignment, on_delete=models.SET_NULL, null=True, related_name="audit_snapshots"
    )
    requirement_node = models.ForeignKey(
        RequirementNode, on_delete=models.CASCADE, related_name="audit_snapshots"
    )
    framework = models.ForeignKey(
        Framework, on_delete=models.CASCADE, related_name="audit_snapshots"
    )
    spoc_user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="snapshot_spoc_assignments"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["compliance_assessment", "spoc_user"]),
            models.Index(fields=["compliance_assessment", "framework"]),
            models.Index(fields=["compliance_assessment", "snapshot_version"]),
        ]


class EvidenceRequirementSnapshot(AbstractBaseModel):
    assessment_control = models.ForeignKey(
        AssessmentControlSnapshot, on_delete=models.CASCADE, related_name="requirement_snapshots"
    )
    source_requirement = models.ForeignKey(
        EvidenceRequirement, on_delete=models.SET_NULL, null=True, blank=True, related_name="snapshots"
    )
    name = models.CharField(max_length=255)
    description = models.TextField(blank=True, default="")
    is_mandatory = models.BooleanField(default=True)
    ...
    class Meta:
        indexes = [
            models.Index(fields=["assessment_control", "is_mandatory"]),
        ]


class AssessmentReviewLevel(AbstractBaseModel):
    assessment_control = models.ForeignKey(
        AssessmentControlSnapshot, on_delete=models.CASCADE, related_name="review_levels"
    )
    reviewer_user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="assessment_review_levels"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(condition=models.Q(level__gte=1), name="review_level_positive")
        ]
        indexes = [
            models.Index(fields=["reviewer_user", "level"]),
        ]


class AuditEvidenceLink(AbstractBaseModel, SoftDeleteMixin):
    class ReviewStatus(models.TextChoices):
        PENDING_REVIEW = "PENDING_REVIEW", _("Pending Review")
        APPROVED = "APPROVED", _("Approved")
        REJECTED = "REJECTED", _("Rejected")

    class SupersededReason(models.TextChoices):
        NONE = "", ""
        NEW_REVISION = "NEW_REVISION", _("Superseded by newer revision")
        RESUBMISSION = "RESUBMISSION", _("Superseded by re-upload after rejection")

    assessment_control = models.ForeignKey(
        AssessmentControlSnapshot, on_delete=models.CASCADE, related_name="evidence_links"
    )
    evidence_requirement_snapshot = models.ForeignKey(
        EvidenceRequirementSnapshot, on_delete=models.SET_NULL, null=True, blank=True, related_name="evidence_links"
    )
    control_evidence_mapping = models.ForeignKey(
        ControlEvidenceMapping, on_delete=models.SET_NULL, null=True, blank=True, related_name="audit_links"
    )
    evidence = models.ForeignKey(
        Evidence, on_delete=models.CASCADE, related_name="audit_links"
    )
    evidence_revision = models.ForeignKey(
        "EvidenceRevision", on_delete=models.CASCADE, related_name="audit_links"
    )
    review_status = models.CharField(
        max_length=20, choices=ReviewStatus.choices, default=ReviewStatus.PENDING_REVIEW
    )
    review_level = models.IntegerField(default=1)
    reviewer_feedback = models.TextField(blank=True, default="")
    reviewed_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="reviewed_audit_evidences"
    )
    reviewed_at = models.DateTimeField(null=True, blank=True)
    version = models.IntegerField(default=1)
    expiry_warning = models.BooleanField(default=False)
    superseded_reason = models.CharField(
        max_length=20, choices=SupersededReason.choices, default=SupersededReason.NONE, blank=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=~(models.Q(review_status="APPROVED") & ~models.Q(superseded_reason="")),
                name="approved_links_not_superseded"
            ),
        ]
        indexes = [
            models.Index(fields=["assessment_control", "review_status"]),
            models.Index(fields=["reviewed_by", "review_status"]),
            models.Index(fields=["evidence", "review_level"]),
            models.Index(fields=["expiry_warning", "review_status"]),
            models.Index(fields=["evidence_revision", "review_status"]),
        ]


class AssignmentChangeLog(AbstractBaseModel):
    class ActionType(models.TextChoices):
        CREATED = "CREATED", _("Assignment Created")
        MODIFIED = "MODIFIED", _("Assignment Modified")
        DEACTIVATED = "DEACTIVATED", _("Assignment Deactivated")
        REACTIVATED = "REACTIVATED", _("Assignment Reactivated")
        BULK_ASSIGNED = "BULK_ASSIGNED", _("Bulk Assignment")

    control_assignment = models.ForeignKey(
        ControlAssignment, on_delete=models.CASCADE, related_name="change_logs"
    )
    action_type = models.CharField(max_length=20, choices=ActionType.choices)
    field_changed = models.CharField(max_length=50, blank=True, default="")
    previous_user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="assignment_changes_from"
    )
    new_user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="assignment_changes_to"
    )
    changed_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="assignment_changes_made"
    )
    reason = models.TextField(blank=True, default="")
    bulk_operation_id = models.UUIDField(null=True, blank=True)

    class Meta:
        indexes = [
            models.Index(fields=["control_assignment", "action_type"]),
            models.Index(fields=["changed_by", "-created_at"]),
            models.Index(fields=["bulk_operation_id"]),
        ]


class AuditCompletion(AbstractBaseModel):
    compliance_assessment = models.ForeignKey(
        ComplianceAssessment, on_delete=models.CASCADE, related_name="completions"
    )
    completion_number = models.IntegerField()
    snapshot_version = models.IntegerField()
    closed_at = models.DateTimeField(auto_now_add=True)
    closed_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="audit_closures"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["compliance_assessment", "completion_number"]),
            models.Index(fields=["closed_at"]),
        ]


class AuditReopenLog(AbstractBaseModel):
    compliance_assessment = models.ForeignKey(
        ComplianceAssessment, on_delete=models.CASCADE, related_name="reopen_logs"
    )
    reopened_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="audit_reopens"
    )
    ...
    class Meta:
        indexes = [
            models.Index(fields=["compliance_assessment", "-reopened_at"]),
        ]


class Notification(AbstractBaseModel):
    class NotificationType(models.TextChoices):
        ASSIGNMENT = "ASSIGNMENT", _("New Control Assignment")
        EVIDENCE_DUE = "EVIDENCE_DUE", _("Evidence Due Soon")
        EVIDENCE_SUBMITTED = "EVIDENCE_SUBMITTED", _("Evidence Submitted for Review")
        EVIDENCE_APPROVED = "EVIDENCE_APPROVED", _("Evidence Approved")
        EVIDENCE_REJECTED = "EVIDENCE_REJECTED", _("Evidence Rejected")
        ESCALATION = "ESCALATION", _("Escalation Alert")
        AUDIT_PHASE_CHANGE = "AUDIT_PHASE_CHANGE", _("Audit Phase Changed")
        INTERMEDIARY = "INTERMEDIARY", _("Intermediary Compliance")
    ...
    class Meta:
        indexes = [
            ...
            models.Index(fields=["user", "related_object_type", "related_object_id", "is_read"]),
            models.Index(fields=["created_at"]),
        ]


class EscalationRule(AbstractBaseModel):
    class TriggerBaseline(models.TextChoices):
        ASSIGNMENT_DATE = "ASSIGNMENT_DATE", _("Assignment Date (Test Mode)")
        EVIDENCE_DUE_DATE = "EVIDENCE_DUE_DATE", _("Evidence Due Date (Production Mode)")

    name = models.CharField(max_length=255)
    description = models.TextField(blank=True, default="")
    is_active = models.BooleanField(default=True)
    trigger_baseline = models.CharField(
        max_length=30, choices=TriggerBaseline.choices, default=TriggerBaseline.ASSIGNMENT_DATE
    )
    escalation_steps = models.JSONField(default=list)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_escalation_rules"
    )


class EscalationLog(AbstractBaseModel):
    class TriggerType(models.TextChoices):
        AUTOMATIC = "AUTOMATIC", _("Automatic (Scheduled)")
        MANUAL = "MANUAL", _("Manual (Button Click)")

    escalation_rule = models.ForeignKey(
        EscalationRule, on_delete=models.SET_NULL, null=True, related_name="logs"
    )
    compliance_assessment = models.ForeignKey(
        ComplianceAssessment, on_delete=models.CASCADE, null=True, blank=True, related_name="escalation_logs"
    )
    control_assignment = models.ForeignKey(
        ControlAssignment, on_delete=models.CASCADE, null=True, blank=True, related_name="escalation_logs"
    )
    ...
    recipient_user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="received_escalations"
    )
    ...
    class Meta:
        unique_together = [
            ("compliance_assessment", "control_assignment", "escalation_rule", "step_label", "escalation_date")
        ]
        indexes = [
            models.Index(fields=["compliance_assessment", "escalation_date"]),
        ]


auditlog.register(ControlAssignment, exclude_fields=common_exclude)
auditlog.register(EvidenceRequirement, exclude_fields=common_exclude)
auditlog.register(ControlEvidenceMapping, exclude_fields=common_exclude)
auditlog.register(AssessmentControlSnapshot, exclude_fields=common_exclude)
auditlog.register(EvidenceRequirementSnapshot, exclude_fields=common_exclude)
auditlog.register(AssessmentReviewLevel, exclude_fields=common_exclude)
auditlog.register(AuditEvidenceLink, exclude_fields=common_exclude)
auditlog.register(AssignmentChangeLog, exclude_fields=common_exclude)
auditlog.register(AuditCompletion, exclude_fields=common_exclude)
auditlog.register(AuditReopenLog, exclude_fields=common_exclude)
auditlog.register(Notification, exclude_fields=common_exclude)
auditlog.register(EscalationRule, exclude_fields=common_exclude)
auditlog.register(EscalationLog, exclude_fields=common_exclude)


class CustomReportTemplate(AbstractBaseModel, FolderMixin):
    """
    Allows admins to configure PDF report template layout, branding, and section settings.
    Supports 'compliance' and 'intermediary_compliance' report types.
    """
    report_type = models.CharField(
        max_length=50,
        unique=True,
        help_text=_("Report type identifier: 'compliance' or 'intermediary_compliance'"),
    )
    title = models.CharField(max_length=255, default="Security Audit Compliance Report")
    header_text = models.CharField(max_length=255, default="CONFIDENTIAL - FOR INTERNAL USE ONLY")
    footer_text = models.CharField(max_length=255, default="Enterprise Compliance & Risk Management Platform")
    company_name = models.CharField(max_length=255, default="Enterprise Compliance Organization")
    ...
    custom_executive_summary_text = models.TextField(blank=True, default="")
    custom_concluding_notes = models.TextField(blank=True, default="")
    custom_findings_title = models.CharField(max_length=255, blank=True, default="")
    custom_evidence_title = models.CharField(max_length=255, blank=True, default="")
    custom_activity_title = models.CharField(max_length=255, blank=True, default="")
    is_active = models.BooleanField(default=True)

    fields_to_check = ["report_type"]

    def __str__(self):
        return f"{self.report_type} report template"


auditlog.register(CustomReportTemplate, exclude_fields=common_exclude)
