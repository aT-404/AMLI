...
class NormalizeContentTypeMiddleware:
    """
    Middleware that normalizes incoming 'text/plain' content-type headers
    for API requests into 'application/json', ensuring DRF parsers handle the request body cleanly.
    """
    ...
    def __call__(self, request):
        ...
        if ct and "text/plain" in ct.lower():
