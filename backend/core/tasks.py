...
def process_pending_spoc_assignment_emails():
    """
    Periodically checks for SPOC users with pending control assignments
    whose 24-hour email cooldown has expired.
    Dispatches EXACTLY ONE consolidated assignment email per SPOC containing all pending controls.
    """
    from datetime import timedelta
    from django.utils import timezone
    from django.db import transaction
    from core.models import ControlAssignment, Notification
    from core.notification_service import EmailChannel

    now = timezone.now()
    cooldown_threshold = now - timedelta(hours=24)

    spocs = list(User.objects.filter(
        spoc_control_assignments__pending_email_notification=True,
    ).distinct())
    print(f"[DEBUG PROCESS] Found {len(spocs)} spocs with pending_email_notification=True")

    for spoc in spocs:
        print(f"[DEBUG PROCESS] Checking spoc {spoc.email}, last_sent={spoc.last_assignment_email_sent_at}, threshold={cooldown_threshold}")
        last_sent = spoc.last_assignment_email_sent_at
        if last_sent and last_sent > cooldown_threshold:
            print(f"[DEBUG PROCESS] Skipping spoc {spoc.email} due to cooldown")
            continue

        pending_assignments = list(
            ControlAssignment.all_objects.filter(
                spoc_user=spoc,
                pending_email_notification=True
            ).select_related("requirement_node", "framework")
        )
        print(f"[DEBUG PROCESS] spoc {spoc.email} pending_assignments count: {len(pending_assignments)}")
        ...
        spoc.last_assignment_email_sent_at = now
        spoc.save(update_fields=["last_assignment_email_sent_at"])

        sent_ids = [pa.id for pa in pending_assignments]
        updated_count = ControlAssignment.all_objects.filter(id__in=sent_ids).update(pending_email_notification=False)
        print(f"[DEBUG PROCESS] Updated {updated_count} assignments for spoc {spoc.email} to pending_email_notification=False")
...
def check_evidences_expired():
    """
    Check for expired Evidences and update status metadata in UI/Repository.
    Does NOT send email prompts or create resubmission requests outside an active audit.
    """
    expired_evidences = Evidence.objects.filter(
        expiry_date__lt=date.today()
    ).exclude(status="expired")

    expired_evidences.update(status="expired")
...
def auto_trigger_scheduled_compliance_assessments():
    """
    Periodically checks for scheduled compliance assessments due for yearly rollover.
    Clones the assessment and shifts dates (start_date, evidence_due_date, due_date/audit_end_date)
    and audit_period_year string (+1 year).
    """
    from core.audit_lifecycle import (
        trigger_next_yearly_assessment,
        shift_date_by_year,
        shift_period_string_by_year,
    )

    scheduled_assessments = ComplianceAssessment.objects.filter(
    ...
    for asm in scheduled_assessments:
        target_next_start = shift_date_by_year(asm.start_date, 1)
        if target_next_start and today >= target_next_start:
            new_audit_period = shift_period_string_by_year(asm.audit_period_year, 1)
            exists = ComplianceAssessment.objects.filter(
            ...
            if not exists:
                new_asm = trigger_next_yearly_assessment(asm, years=1)
                created_count += 1
                logger.info(
                    f"Auto-triggered yearly scheduled compliance assessment: {new_asm.name} ({new_asm.audit_period_year})"
                )
