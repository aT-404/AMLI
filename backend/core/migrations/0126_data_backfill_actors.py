...
def backfill_actors(apps, schema_editor):
    ...
    existing_uids = set(Actor.objects.filter(user_id__isnull=False).values_list("user_id", flat=True))
    user_ids = [uid for uid in User.objects.values_list("id", flat=True) if uid not in existing_uids]
    if user_ids:
    ...
    existing_eids = set(Actor.objects.filter(entity_id__isnull=False).values_list("entity_id", flat=True))
    entity_ids = [eid for eid in Entity.objects.values_list("id", flat=True) if eid not in existing_eids]
    if entity_ids:
        Actor.objects.bulk_create(
    ...
    for app_label, model_name, old_field, new_field in M2M_MAPPINGS:
        ...
        if new_relations:
            if "mssql" in schema_editor.connection.vendor:
                NewThrough.objects.bulk_create(new_relations, batch_size=5000)
            else:
