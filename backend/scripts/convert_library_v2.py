...
def _normalize_header_col(col_name: Any) -> str:
    col = str(col_name).strip().lower() if col_name is not None else ""
    if col in ("ref", "ref id", "ref_id", "code", "id", "item", "no", "number", "reference", "req id", "requirement id"):
        return "ref_id"
    if col in ("title", "name", "requirement", "control", "control name", "rule", "item name", "heading"):
        return "name"
    if col in ("description", "details", "text", "specification", "summary", "explanation", "requirement description"):
        return "description"
    if col in ("domain", "category", "section", "group", "family"):
        return "implementation_groups"
    if any(k in col for k in ("evidence", "proof")):
...
def parse_content_rows(content_ws):
    ...
    if not rows:
        return [], []
    raw_header = [str(cell.value).strip().lower() if cell.value else "" for cell in rows[0]]
    norm_header = [_normalize_header_col(cell.value) for cell in rows[0]]
    ...
    for row in rows[1:]:
        ...
        for i in range(len(raw_header)):
            if i < len(row):
                val = row[i].value
                if raw_header[i]:
                    data[raw_header[i]] = val
                if norm_header[i] and norm_header[i] not in data:
                    data[norm_header[i]] = val
        rows_with_data.append((row, data))
    return norm_header, rows_with_data
...
def _handle_framework(obj, library, object_blocks, prefix_to_urn, compat_mode, verbose):
    ...
    rows = list(content_ws.iter_rows())
    if rows:
        raw_header = [str(c.value).strip().lower() if c.value else "" for c in rows[0]]
        header = [_normalize_header_col(c.value) for c in rows[0]]

        # Pre-pass: count occurrences of every ref_id to auto-disambiguate duplicates
        ref_id_raw_counts = Counter()
        for row in rows[1:]:
            d_tmp = {}
            for i in range(len(raw_header)):
                if i < len(row):
                    v = row[i].value
                    if raw_header[i]:
                        d_tmp[raw_header[i]] = v
                    if header[i] and header[i] not in d_tmp:
                        d_tmp[header[i]] = v
            if any(value is not None and value != "" for value in d_tmp.values()):
                r_id = d_tmp.get("ref_id")
                if r_id is not None and str(r_id).strip():
                    ref_id_raw_counts[str(r_id).strip().lower()] += 1
        ...
        for row in rows[1:]:
            ...
            for i in range(len(raw_header)):
                if i < len(row):
                    ...
                    if raw_header[i]:
                        data[raw_header[i]] = v
                    if header[i] and header[i] not in data:
                        data[header[i]] = v
            if all(value is None or value == "" for value in data.values()):
                print(f"empty line {counter}")
                continue
            depth_val = data.get("depth")
            depth = int(depth_val) if depth_val is not None and str(depth_val).strip().isdigit() else 1
            ref_id = data.get("ref_id")
            ref_id = str(ref_id).strip() if ref_id is not None else None

            # Disambiguate duplicate ref_ids (e.g. A.1 appearing 3 times -> A.1.1, A.1.2, A.1.3; B.3 x2 -> B.3.1, B.3.2)
            if ref_id:
                ref_id_key = ref_id.lower()
                if ref_id_raw_counts[ref_id_key] > 1:
                    ref_id_seen[ref_id_key] += 1
                    ref_id = f"{ref_id}.{ref_id_seen[ref_id_key]}"
                    data["ref_id"] = ref_id
            ...
            orig_urn = node.get("urn")
            target_urn = orig_urn
            urn_suffix_idx = 1
            while target_urn in all_urns:
                urn_suffix_idx += 1
                target_urn = f"{orig_urn}-{urn_suffix_idx}"
            node["urn"] = target_urn
            all_urns.add(target_urn)
            requirement_nodes.append(node)

        # Domain-wise Expandable Hierarchy Transformation
        # If all nodes are flat (no parent_urn and depth == 1), group them by Domain/Category
        has_parents = any(n.get("parent_urn") for n in requirement_nodes)
        if not has_parents and requirement_nodes:
            from collections import OrderedDict
            def _normalize_domain_title(raw_dom: str) -> str:
                if not raw_dom:
                    return "General Requirements"
                s = str(raw_dom).strip()

                # Strip trailing colons, semicolons, dashes, or punctuation
                s = re.sub(r'[\:\;\,\-\_\.]+$', '', s).strip()

                # Strip parenthetical codes/abbreviations like (Rc.Co), (Rs.Co), (WFRL), (Pr.Pt), etc. (handling optional trailing colons)
                s = re.sub(r'\s*\([A-Za-z0-9\.\_\-\s]+\)\s*(?:[\:\;\,\-\_\.]*)?$', '', s, flags=re.IGNORECASE).strip()

                # Strip trailing " Domain" or "- Domain"
                s = re.sub(r'\s*[\-\–\—]?\s*Domain\s*(?:[\:\;\,\-\_\.]*)?$', '', s, flags=re.IGNORECASE).strip()
                s = re.sub(r'[\:\;\,\-\_\.]+$', '', s).strip()
                s = re.sub(r'\s+', ' ', s)
                return s.title() if s else "General Requirements"

            norm_domain_map = OrderedDict()
            for n in requirement_nodes:
                groups = n.get("implementation_groups") or []
                raw_dom = groups[0] if groups else "General Requirements"
                norm_title = _normalize_domain_title(raw_dom)
                norm_key = norm_title.lower()

                if norm_key not in norm_domain_map:
                    norm_domain_map[norm_key] = {"title": norm_title, "nodes": []}
                norm_domain_map[norm_key]["nodes"].append(n)

            if len(norm_domain_map) > 1 or (len(norm_domain_map) == 1 and "general requirements" not in norm_domain_map):
                new_nodes = []
                dom_idx = 0
                for norm_key, dom_data in norm_domain_map.items():
                    dom_idx += 1
                    dom_name = dom_data["title"]
                    child_nodes = dom_data["nodes"]

                    dom_slug = clean_urn_suffix(dom_name, compat_mode=compat_mode) or f"dom-{dom_idx}"
                    dom_urn = f"{base_urn}:dom-{dom_slug}"

                    t_dom_urn = dom_urn
                    d_idx = 1
                    while t_dom_urn in all_urns:
                        d_idx += 1
                        t_dom_urn = f"{dom_urn}-{d_idx}"
                    dom_urn = t_dom_urn
                    all_urns.add(dom_urn)

                    # Domain Header Node (depth=1, assessable=False, description=None so no redundant byline renders)
                    dom_node = {
                        "urn": dom_urn,
                        "ref_id": f"DOM-{dom_idx}",
                        "name": dom_name,
                        "description": None,
                        "depth": 1,
                        "assessable": False,
                        "implementation_groups": [dom_name]
                    }
                    new_nodes.append(dom_node)

                    # Child Requirement Nodes under domain (depth=2, assessable=True, parent_urn=dom_urn)
                    c_idx = 0
                    for child in child_nodes:
                        c_idx += 1
                        child["parent_urn"] = dom_urn
                        child["depth"] = 2
                        child["assessable"] = True
                        if not child.get("implementation_groups"):
                            child["implementation_groups"] = [dom_name]

                        # Clean control numbering: set ref_id cleanly without duplicating in name
                        raw_ref = child.get("ref_id")
                        ctrl_num = str(raw_ref).strip() if raw_ref and str(raw_ref).strip() else f"{dom_idx}.{c_idx}"
                        child["ref_id"] = ctrl_num

                        curr_name = child.get("name") or child.get("description") or "Requirement"
                        # Strip any duplicate leading ref_id or ctrl_num from curr_name
                        curr_name = re.sub(r'^(?:' + re.escape(ctrl_num) + r'|' + re.escape(str(raw_ref or '')) + r')\s*[\-\:\.]\s*', '', curr_name, flags=re.IGNORECASE).strip()
                        child["name"] = curr_name

                        new_nodes.append(child)

                requirement_nodes = new_nodes
...
def create_library(
    ...
    if raw_stem.lower().startswith("custom_lib_"):
        raw_stem = raw_stem[11:]
    clean_title = re.sub(r'_[a-zA-Z0-9]{6,10}$', '', raw_stem).replace("_", " ").replace("-", " ").strip().title()
    if not clean_title or clean_title.lower() in ("library", "file", "sheet", "input"):
        clean_title = "Custom Security Checklist"

    file_stem = clean_urn_suffix(clean_title, compat_mode=0) or "custom_checklist"
    lib_default_urn = f"urn:custom:compliance:library:{file_stem}"

    # Step 1: Load library_meta (with fallback if sheet is missing)
    if "library_meta" in sheets:
        ...
        library_meta = {
            ...
            "urn": lib_default_urn,
            "name": clean_title,
            "description": f"{clean_title} Security Framework & Checklist",
            "locale": "en",
            "ref_id": file_stem,
            "version": 1,
            "publication_date": datetime.datetime.now().date(),
            "provider": "Custom Upload",
            "packager": "Custom Upload",
        }

    # Ensure URN matches standard 5-part format urn:packager:domain:type:ref_id
    existing_urn = library_meta.get("urn")
    if existing_urn:
        parts = existing_urn.split(":")
        if len(parts) == 4 and parts[0] == "urn":
            library_meta["urn"] = f"urn:{parts[1]}:compliance:{parts[2]}:{parts[3]}"
        elif not existing_urn.startswith("urn:"):
            library_meta["urn"] = lib_default_urn
    else:
        library_meta["urn"] = lib_default_urn

    if library_meta.get("type") != "library":
        library_meta["type"] = "library"
    if not library_meta.get("name"):
        library_meta["name"] = file_path.stem.replace("_", " ").replace("-", " ").title()
    if not library_meta.get("version"):
        library_meta["version"] = 1
    if not library_meta.get("locale"):
        library_meta["locale"] = "en"
    if not library_meta.get("ref_id"):
        library_meta["ref_id"] = file_stem
    ...
    if not object_blocks:
        ...
        for sname in sheets:
            if sname != "library_meta":
                ws = wb[sname]
                rows = list(ws.iter_rows(values_only=True))
                if len(rows) > 1:
                    candidate_sheet = ws
                    break
        if candidate_sheet is None and len(sheets) > 0:
            candidate_sheet = wb[sheets[0]]

        if candidate_sheet:
            fw_urn = f"urn:custom:compliance:framework:{file_stem}"
            object_blocks["framework"] = {
                "type": "framework",
                "meta": {
                    "type": "framework",
                    "urn": fw_urn,
                    ...
                    "description": f"Framework from {file_path.name}",
                    "ref_id": file_stem,
                    "locale": "en",
                },
                "content_sheet": candidate_sheet,
            }
