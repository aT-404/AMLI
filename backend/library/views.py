...
class StoredLibraryViewSet(BaseModelViewSet):
    ...
    def get_queryset(self) -> models.query.QuerySet:
        if not self.request or not self.request.user or not self.request.user.is_authenticated:
            return StoredLibrary.objects.none()
    ...
    def retrieve(self, request, *args, pk, **kwargs):
        ...
        serializer = self.get_serializer(lib)
    ...
    def content(self, request, pk):
        ...
        content_data = update_translations(lib.content or {})
        obj_type = request.query_params.get("object_type")
        if obj_type and isinstance(content_data, dict):
            return Response({obj_type: content_data.get(obj_type, [])})
    ...
    def tree(self, request, pk):
        ...
        library_objects = lib.content or {}
        framework = library_objects.get("framework") if isinstance(library_objects, dict) else None
        ...
        if framework:
            try:
                ...
                logger.error("Error generating tree view from content", error=e, pk=pk)

        # Fallback to persisted Framework and RequirementNodes for loaded libraries
        loaded_lib = lib.get_loaded_library()
        fw = Framework.objects.filter(library=loaded_lib).first() if loaded_lib else None
        if not fw:
            fw_slug = lib.urn.split(":")[-1]
            fw = Framework.objects.filter(urn__icontains=fw_slug).first()

        if fw:
            nodes = RequirementNode.objects.filter(framework=fw).select_related("folder")
            return Response(get_sorted_requirement_nodes(nodes, None, None))
...
def _get_readable_stored_library(user, pk_or_urn) -> StoredLibrary | None:
    """Visibility policy for global StoredLibrary catalog templates.

    Superusers have full access.
    Authenticated users have read/tree access to catalog libraries.
