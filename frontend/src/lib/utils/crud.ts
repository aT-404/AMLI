...
export const getOptions = ({
	...
	const options = objects
		.map((object) => {
			let rawLabel = object[label];
			if (label === 'user_name_designation') {
				const fullName = [object['first_name'], object['last_name']].filter(Boolean).join(' ').trim();
				const desig = object['designation'] ? String(object['designation']).trim() : '';
				if (fullName && desig) {
					rawLabel = `${fullName} - ${desig}`;
				} else if (fullName) {
					rawLabel = fullName;
				} else if (object['email'] && desig) {
					rawLabel = `${object['email']} - ${desig}`;
				} else if (object['email']) {
					rawLabel = object['email'];
				}
			}
			const fallbackLabel = object['name'] || object['str'] || object['description'] || object['email'] || '';
			const my_label =
				label != 'auto'
					? (rawLabel && String(rawLabel).trim() !== '' ? rawLabel : fallbackLabel)
					: (append(object['ref_id'], object['name'] ? object['name'] : object['description']) || fallbackLabel);
			return {
				label:
