<script lang="ts">
	...
	function createCalendarEvents(): CalendarEvent[] {
		const events: CalendarEvent[] = [
			...
			...(data.audits ?? []).flatMap((audit: any) => {
				const auditEvents: CalendarEvent[] = [];
				if (audit.due_date) {
					auditEvents.push({
						label: `[Audit End] ${audit.name}`,
				...
				if (audit.start_date) {
					auditEvents.push({
						label: `[Audit Start] ${audit.name}`,
						date: localDate(audit.start_date),
						link: `/compliance-assessments/${audit.id}`,
						users: audit.authors ?? [],
						category: 'audit' as const,
						status: audit.status
				...
				if (audit.evidence_due_date) {
					auditEvents.push({
						label: `[Evidence Due] ${audit.name}`,
						date: localDate(audit.evidence_due_date),
						link: `/compliance-assessments/${audit.id}`,
						users: audit.authors ?? [],
						category: 'audit' as const,
						status: audit.status
