...
<div
	class="flex items-center gap-3 py-2 px-3 rounded-xl transition-all -ml-2
		{assessable
		? isLocked
			? 'bg-surface-100-900/40 border border-surface-200-800/60 text-surface-400 opacity-60'
			: isChecked
				? 'bg-violet-500/20 border border-violet-500/40 text-surface-900-100'
				: 'bg-surface-100-900/80 border border-surface-200-800 hover:bg-surface-200-800/80 text-surface-900-100 cursor-pointer'
		: ''}"
...
>
	...
	{#if assessable}
		...
		<input
			type="checkbox"
			class="checkbox checkbox-sm {isLocked ? '' : 'border-surface-400 checked:bg-violet-500'}"
			checked={isChecked}
			disabled={isLocked}
	...
	<div class="flex flex-col flex-1">
		<div class="flex items-center gap-2">
			...
			{#if assessable}
				{#if isLocked}
					<span class="text-surface-400" title={m.alreadyAssigned()}>
